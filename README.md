...
Портативный инсталлятор для запуска **ComfyUI** с поддержкой генерации 3D через **TRELLIS2 (GGUF)** из коробки.
...
4. **Запустите** Отредактируйте под свою конфигурацию компа `run_nvidia_gpu.bat` и запустите
5. Профит :)
...
- Модели: DINOv3, Trellis2 GGUF (Q4_K_M)
...
├── comfy_env/              # Conda-окружение (создаётся)
